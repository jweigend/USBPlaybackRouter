...
class SessionController:
    ...
    def observe(self, st):
        ...
        if st.device is None:
            if self.node.running:
                self.node.stop()            # device gone: no node, default falls back
                self.device_node_id = None
                return True
            return False
        if not self.node.running or st.device.node.id != self.device_node_id:
            if now < self.restart_after:
                return False
            return self._start_for(st.device)   # (re)appeared: fresh node on the remembered pair
        if st.source is None:
            return False
        want = self.remembered_pair(st.device)
        if want and (st.pair is None or st.pair.key != want) and st.code != "ambiguous-user":
            if now < self.restart_after:
                return False
            self.restart_after = now + 2.0
            ok, _ = self.backend.select_pair(want, remember=False)
            return ok
        return False
